...
function Scene({ top, mouse }) {
  ...
  return (
    <>
      ...
      <Text opacity={top.interpolate([0, 200], [1, 0])} position={top.interpolate((top) => [0, 2 + top / 200, 0])}>
        E-LOG
      </Text>
      <Text opacity={top.interpolate([0, 200], [1, 0])} position={top.interpolate((top) => [0, -1 + top / 200, 0])}>
        DEF
      </Text>
      <Text opacity={top.interpolate([0, 200], [1, 0])} position={top.interpolate((top) => [0, 0.5 + top / 200, 0])}>
        EGR
      </Text>
      <Text opacity={top.interpolate([0, 200], [1, 0])} position={top.interpolate((top) => [0, -2.5 + top / 200, 0])}>
        DPF
      </Text>
      <Text opacity={top.interpolate([0, 200], [1, 0])} position={top.interpolate((top) => [0, -3.8 + top / 400, 0])}>
        (scroll&dArr;)
      </Text>
      <Text position={top.interpolate((top) => [0, -18 + ((top * 10) / scrollMax) * 2, 0])} color="black" fontSize={150}>
        They broken again?
      </Text>
      ...
      <Text position={top.interpolate((top) => [0, -21 + ((top * 10) / scrollMax) * 2, 0])} color="black" fontSize={150}>
        Check Inventory
      </Text>
    </>
  )
